...
def decrypt_message(keys):
    loadfile = open('message_test.enc', "rb")
    file = loadfile.read()
    for key in keys:
        decrypted_private = decrypt(file, key[1])
        if type(decrypted_private) == str:
            print(key[0] + ': ' + str(decrypted_private))
