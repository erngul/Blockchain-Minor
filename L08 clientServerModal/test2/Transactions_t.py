...
if __name__ == "__main__":
  ...
  # Check Invalid Transactions
    ...
    for transaction in [Tx3, Tx4, Tx5, Tx6, Tx7, Tx8]:
        ...
        time.sleep(1)
        s.send(pickle.dumps(transaction.outputs))
        time.sleep(1)
        s.send(pickle.dumps(transaction.sigs))
        if transaction.reqd != None:
            s.send(pickle.dumps(transaction.reqd))
        else:
            s.send(pickle.dumps(None))
        time.sleep(1)
